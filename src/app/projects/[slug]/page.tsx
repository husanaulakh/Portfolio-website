...
export async function generateMetadata({ params }: ProjectPageProps) {
  ...
  return {
    title: `${project.title} | Neon Canvas`,
  ...
  return (
    <div className="space-y-10 py-8">
      <header className="text-center">
        <Button asChild variant="ghost" size="sm" className="mb-4 text-muted-foreground hover:text-accent">
          <Link href="/projects">
            <ArrowLeft className="mr-2 h-4 w-4" /> Back to All Projects
          </Link>
        </Button>
        <h1 className="font-serif text-5xl font-bold mb-2">{project.title}</h1>
        <p className="text-xl text-primary">{project.tagline}</p>
      </header>

      <div className="relative w-full aspect-[16/9] rounded-lg overflow-hidden shadow-xl max-h-[500px] mx-auto group">
        <Image
      ...
      <div className="grid lg:grid-cols-3 gap-8 items-start">
        <main className="lg:col-span-2 space-y-6">
          <Card className="bg-card/50 shadow-md">
            <CardHeader>
              <CardTitle className="font-serif text-3xl">Project Overview</CardTitle>
            </CardHeader>
            <CardContent>
              <div
        ...
        <aside className="lg:col-span-1 space-y-6 lg:sticky lg:top-24">
          <Card className="bg-card/50 shadow-md">
            <CardHeader>
              <CardTitle className="font-serif text-2xl">Tech Stack</CardTitle>
            </CardHeader>
          ...
          {(project.repoUrl || project.liveUrl) && (
            <Card className="bg-card/50 shadow-md">
              <CardHeader>
                <CardTitle className="font-serif text-2xl">Links</CardTitle>
              </CardHeader>
           ...
           {project.detailsImageUrl && (
            <Card className="bg-card/50 shadow-md">
                <CardHeader>
                    <CardTitle className="font-serif text-2xl">Details</CardTitle>
                    <CardDescription>Visual Aid / Schematic</CardDescription>
                </CardHeader>
                <CardContent>
                    <div className="relative w-full aspect-[4/3] rounded-md overflow-hidden bg-secondary/20">
                         <Image
